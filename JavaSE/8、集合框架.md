...
* **`loadFactory`**：加载因子，用于判断当前是否需要进行扩容操作，默认值为DEFAULT_LOAD_FACTOR，即0.75。
...
> **当`(size >= threshold) && (null != table[i])`时，即当元素的个数达到临界值`threshold`时，并且要插入的数组位置有元素存在，此时才会去扩容。**

比如：

![image-20240108164300943](.\images\image-20240108164300943.png)

如上图所示，若集合中的元素已经达到了临界值，此时插入到数组中（红色箭头所表示的），依旧不会导致扩容；若插入到链表中（蓝色箭头所表示的），就会去扩容。
