...
外部类可以进行修饰的只有缺省、public和abstract。但是内部类可以使用private、protected以及static进行修饰。这一点，也就是我将其看成是外部类的一部分的原因，方法也是可以声明成private、protected以及static的。
...
其中最常用的是@Retention和@Target，下面分别介绍一下这五种元注解。
...
![image-20231130215144910](.\images\image-20231130215144910.png)

当满足i >= IntegerCache.low && i <= IntegerCache.high时，获得的Integer对象从现有的cache数组中获取的。其中Integer包装类中low的值是-128，high的值是127。

让我们来看看cache数组：

![image-20231130215726147](.\images\image-20231130215726147.png)
