...
代码优化一下：在synchronized代码块之前再设置一个判断，让其他线程在进入之前不用进行等待
...
> 内部类只有在使用到内部类的时候才加载，产生INSTANCE实例；又不用加锁。
>
> 故对于上例来说，当我们一开始使用getInstance()方法时，才会去加载内部类Inner，才会去创建LazySingle()对象；此时即不会有饿汉式一开始边创建对象占用内存的缺点，也不会有一般懒汉式的线程安全问题，因为一调用方法，就会加载内部类，在加载类的时候不会出现线程安全问题。
>
