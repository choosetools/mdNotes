...
当满足一定条件或者手动执行bgsave命令，就可以进行异步地RDB持久化。异步持久化就是去开启一个子进程，由子进程去完成内存的读取并写入磁盘RDB文件的操作。由于该持久化是异步的，所以几乎对主线程没有影响。但是，子进程是由主进程fork得来的，这个fork的过程是阻塞的，此时主进程只能进行fork操作，不能处理其他请求，因此我们需要尽快完成fork，避免阻塞时间过久。
